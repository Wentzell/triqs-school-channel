...
do
  # TRIQS 4.0 migrated cpp2py -> c2py, so several of these files (bin/triqs++,
  # bin/nda++, the Cpp2Py cmake/python bits) are no longer installed. Skip any
  # that are absent rather than letting sed's errexit kill the build.
